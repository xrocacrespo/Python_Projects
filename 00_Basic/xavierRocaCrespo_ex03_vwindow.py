...
def apagarHandBar():
    mc.textFieldGrp('windowXavi|colLayout1|fieldheightHandBar', e=True, en=False)
    mc.textFieldGrp('windowXavi|colLayout1|fieldradiusHandBar', e=True, en=False)
    mc.textFieldGrp('windowXavi|colLayout1|fieldradiusUpBars', e=True, en=False)
    mc.textFieldGrp('windowXavi|colLayout1|fieldradiusHandleSpheres', e=True, en=False)

def encenderHandBar():
    mc.textFieldGrp('windowXavi|colLayout1|fieldheightHandBar', e=True, en=True)
    mc.textFieldGrp('windowXavi|colLayout1|fieldradiusHandBar', e=True, en=True)
    mc.textFieldGrp('windowXavi|colLayout1|fieldradiusUpBars', e=True, en=True)
    mc.textFieldGrp('windowXavi|colLayout1|fieldradiusHandleSpheres', e=True, en=True)

def redoSettings(settings, *args,**kwargs):
    groupName = settings['groupName'] = mc.window('windowXavi', q=True, title=True)
    nameStep=settings['nameStep']= mc.textFieldGrp('windowXavi|colLayout1|fieldnameStep', q=True, tx=True),
    numSteps=settings['numSteps']= mc.intSliderGrp('windowXavi|colLayout1|fieldnumSteps', q=True, v=True),
    widthStep=settings['widthStep']= mc.floatSliderGrp('windowXavi|colLayout1|fieldwidthStep', q=True, v=True),
    heightStep=settings['heightStep']= mc.floatSliderGrp('windowXavi|colLayout1|fieldheightStep', q=True, v=True),
    depthStep=settings['depthStep']= mc.floatSliderGrp('windowXavi|colLayout1|fielddepthStep', q=True, v=True),
    offsetStep=settings['offsetStep']= mc.floatSliderGrp('windowXavi|colLayout1|fieldoffsetStep', q=True, v=True),
    radius=settings['radius']= mc.floatSliderGrp('windowXavi|colLayout1|fieldradius', q=True, v=True),
    degrees=settings['degrees']= mc.floatSliderGrp('windowXavi|colLayout1|fielddegrees', q=True, v=True),
    heightHandBar=settings['heightHandBar']= mc.textFieldGrp('windowXavi|colLayout1|fieldheightHandBar', q=True, tx=True),
    radiusHandBar=settings['radiusHandBar']= mc.textFieldGrp('windowXavi|colLayout1|fieldradiusUpBars', q=True, tx=True),
    createHandBar=settings['createHandBar']= mc.checkBoxGrp('windowXavi|colLayout1|rowHandBar|fieldcreateHandBar', q=True, v1=True),
    radiusUpBars=settings['radiusUpBars']= mc.textFieldGrp('windowXavi|colLayout1|fieldradiusUpBars', q=True, tx=True),
    radiusHandleSpheres=settings['radiusHandleSpheres']= mc.floatSliderGrp('windowXavi|colLayout1|fieldradiusHandleSpheres', q=True, v=True),
    createCenterCol=settings['createCenterCol']= mc.checkBoxGrp('windowXavi|colLayout1|rowCenterCol|fieldcreateCenterCol', q=True, v1=True)
    randomPos=mc.checkBoxGrp('windowXavi|colLayout1|rowRandomPos|fieldRandPos', q=True, v1=True)

    tempGroup = xavierRocaCrespo_ex2(
        groupName = groupName,
        nameStep=str(nameStep),
        numSteps=int(numSteps[0]),
        widthStep=float(widthStep[0]),
        heightStep=float(heightStep[0]),
        depthStep=float(depthStep[0]),
        offsetStep=float(offsetStep[0]),
        radius=float(radius[0]),
        degrees=float(degrees[0]),
        heightHandBar=float(heightHandBar[0]),
        radiusHandBar=float(radiusHandBar[0]),
        createHandBar=createHandBar[0],
        radiusUpBars=float(radiusUpBars[0]),
        radiusHandleSpheres=float(radiusHandleSpheres[0]),
        createCenterCol=createCenterCol,
        windowMode=False
    )
    if randomPos:
        mc.xform(tempGroup, t=[rnd.randrange(-100.0, 50.0, 10.0), 0, rnd.randrange(-100.0, 50.0, 10.0)],
...
def xavierRocaCrespo_ex2(groupName='stairs',
    ...
    if windowMode:
        ...
        window = mc.window(winID, title=groupName, iconName='Short Name', resizeToFitChildren=True)
        ...
        fieldnumSteps = mc.intSliderGrp('fieldnumSteps', l='Number Steps', cal=[1, 'right'], v=20, f=True, fieldMaxValue=500, maxValue=500)
        ...
        mc.text('HAND BAR')
        mc.rowLayout('rowHandBar', nc=2)
        mc.text(l='                                                                     ')
        fieldcreateHandBar = mc.checkBoxGrp('fieldcreateHandBar', l='Hand Bar      ', v1=True, cal=[2, 'right'])
        mc.setParent("..")

        fieldheightHandBar = mc.floatSliderGrp('fieldheightHandBar', l='Height', cal=[1, 'right'], v=4, f=True, fieldMaxValue=100,
                          maxValue=10)
        fieldradiusHandBar = mc.floatSliderGrp('fieldradiusHandBar', l='Radius', cal=[1, 'right'], v=0.2, f=True,
                                               fieldMaxValue=100,
                                               maxValue=10)
        mc.text(l='')

        mc.text('UP BARS')
        fieldradiusUpBars = mc.floatSliderGrp('fieldradiusUpBars', l='Radius', cal=[1, 'right'], v=0.1, f=True,
                                               fieldMaxValue=100,
                                               maxValue=10)
        mc.text(l='')

        mc.text('SPHERES')
        fieldradiusHandleSpheres = mc.floatSliderGrp('fieldradiusHandleSpheres', l='Radius', cal=[1, 'right'], v=1, f=True,
                                              fieldMaxValue=100,
                                              maxValue=10)
        mc.text(l='')

        mc.text('CENTER COLUMN')
        mc.rowLayout('rowCenterCol', nc=2)
        mc.text(l='                                                                     ')
        fieldcreateCenterCol = mc.checkBoxGrp('fieldcreateCenterCol', l='Center Column    ', v1=True, cal=[2, 'right'])
        mc.setParent("..")
        mc.text(l='')

        mc.text('RANDOM POSITION')
        mc.rowLayout('rowRandomPos', nc=2)
        mc.text(l='                                                                     ')
        fieldRandPos = mc.checkBoxGrp('fieldRandPos', l='Random Pos    ', v1=True, cal=[2, 'right'])
        mc.setParent("..")

        mc.text(l='')
        mc.text(l='')

        createButton = mc.button(label='Create Stairs', command=('redoSettings(settings)'))
        ...
        if settings['createHandBar']:
            ...
            mc.select('{}upBar*'.format(moduleName))
            mc.delete()
            mc.select('{}*LOC'.format(moduleName))
            mc.delete()
...
exec3 = xavierRocaCrespo_ex2(groupName='stairs2', windowMode=True)
